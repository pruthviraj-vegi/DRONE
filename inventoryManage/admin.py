...
class TransactionDetailAdmin(admin.ModelAdmin):
    list_display = ("name", "from_branch", "to_branch", "created_at", "updated_at")
    search_fields = ("name",)
    list_filter = ("from_branch", "to_branch")
    autocomplete_fields = ("from_branch", "to_branch")


@admin.register(BranchInventory)
class BranchInventoryAdmin(admin.ModelAdmin):
    list_display = ("inventory", "branch", "quantity", "created_at", "updated_at")
    search_fields = ("inventory__part_name",)
    list_filter = ("branch",)
...
class BranchInventoryTransactionAdmin(admin.ModelAdmin):
    ...
    search_fields = (
        "branch_inventory__inventory__part_name",
